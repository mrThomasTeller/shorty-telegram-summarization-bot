import { myTgUser, myTgGroupId, otherTgUser, createSummarizeCommandMessage, createTgMessages, } from '../../lib/tgUtils.js';
import _ from 'lodash';
import { mapTgMessagesToDbMessages } from '../../lib/dbUtils.js';
import { expectBotSentExactMessagesToTg, expectBotCreatedUsers, expectBotCreatedDbChatMessages, expectBotQueriedSummaryFromGpt, expectBotAddedSummariesToDb, } from '../../lib/expectations.js';
...
describe('summarizeBotServer summarize command regular work', () => {
    it('with 3 messages (1 summary part, 1 point)', testCorrectSummary({
        messages: 3,
        summaryPartsCount: 1,
        summaryPartPointsCount: 1,
    }));
    it('with 6 messages (1 summary part, 2 point)', testCorrectSummary({
        messages: 6,
        summaryPartsCount: 1,
        summaryPartPointsCount: 2,
    }));
    it('with 9 messages (1 summary part, 3 point)', testCorrectSummary({
        messages: 9,
        summaryPartsCount: 1,
        summaryPartPointsCount: 3,
    }));
    it('with 12 messages (1 summary part, 4 points)', testCorrectSummary({
        messages: 12,
        summaryPartsCount: 1,
        summaryPartPointsCount: 4,
    }));
    it('with 15 messages (1 summary part, 5 points)', testCorrectSummary({
        messages: 15,
        summaryPartsCount: 1,
        summaryPartPointsCount: 5,
    }));
    it('with 40 messages (2 summary parts, 4 points)', testCorrectSummary({
        messages: 40,
        summaryPartsCount: 2,
        summaryPartPointsCount: 4,
    }));
    it('with 70 messages (3 summary parts, 3 points)', testCorrectSummary({
        messages: 70,
        summaryPartsCount: 3,
        summaryPartPointsCount: 3,
    }));
    it('with 90 messages (4 summary parts, 2 points)', testCorrectSummary({
        messages: 90,
        summaryPartPointsCount: 2,
        summaryPartsCount: 4,
    }));
    it('with 120 messages (5 summary parts, 2 points, makes not less than 2 points)', testCorrectSummary({
        messages: 120,
        summaryPartPointsCount: 2,
        summaryPartsCount: 5,
    }));
    it('with 15 actual and 15 outdated messages (should take only actual messages)', testCorrectSummary({
        messages: [
    ...
    it("last summary was queried more than 24 hours ago, so it shouldn't be taken into summary", testCorrectSummary({
        messages: [
            { date: daysAgo(2), count: 15, shouldBeSkipped: true },
            { date: new Date(), count: 15 },
        ],
    ...
    it("don't take into new summary messages sent before last summary", testCorrectSummary({
        messages: [
...
function testCorrectSummary({ messages, lastSummaryDate, summaryPartPointsCount, summaryPartsCount, }) {
    return async () => {
        ...
        const gptTestSummariesWithReEnumeratedPoints = _.range(gptTestSummariesCount).map((page) => gptTestSummary(page, summaryPartPointsCount));
        // mocks
        if (lastSummaryDate) {
            db.summaries = [
                {
