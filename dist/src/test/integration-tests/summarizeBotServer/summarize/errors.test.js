import { myTgUser, myTgGroupId, createSummarizeCommandMessage, createTgMessages, } from '../../lib/tgUtils.js';
...
describe('summarizeBotServer summarize command errors', () => {
    ...
    it('with unexpected error from gpt', async () => {
        ...
        await simulateChatMessage(createSummarizeCommandMessage(myTgUser));
        // expectations
        expectBotSentExactMessagesToTg(telegramBot, [t('summarize.message.start'), t('summarize.errors.queryProcess')], myTgGroupId);
    });
...
const createGptSummaryError = () => new ChatGPTError('gpt summary error');
function createSummaries(chatId, actualCount, outdatedCount = 0) {
    return [
