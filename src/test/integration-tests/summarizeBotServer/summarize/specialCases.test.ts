...
describe('summarizeBotServer summarize command special cases', () => {
  it('two summaries from two different chats at the same time is ok', async () => {
    ...
    const dbMessages = mapTgMessagesToDbMessages(
      createTgMessages(10, myTgGroupId)
    );
    const dbMessages2 = mapTgMessagesToDbMessages(
      createTgMessages(10, myTgGroup2Id)
    );
    ...
    await simulateChatMessage(
      createSummarizeCommandMessage(myTgUser, myTgGroupId)
    );
    await setTimeout(10);
    await simulateChatMessage(
      createSummarizeCommandMessage(myTgUser, myTgGroup2Id)
    );
    await setTimeout(50);
  ...
  it('will not change order of summaries if second summary came earlier than first', async () => {
    ...
    gpt.sendMessage.mockResolvedValueOnce(
      createGptChatMessage(gptTestSummary(1, pointsCount, 0))
    );
  ...
  it("shouldn't generate more than 5 summary parts", async () => {
    ...
    for (const page of allowedPagesRange) {
      gpt.sendMessage.mockResolvedValueOnce(
        createGptChatMessage(gptTestSummary(page, 2, 0))
      );
    }
    ...
    expectBotSentExactMessagesToTg(
      ...
      [
        ...
        t('summarize.message.header'),
        ...allowedPagesRange.map((page, index) =>
          gptTestSummary(page, 2, index)
        ),
        t('summarize.message.end'),
      ],
      myTgGroupId
